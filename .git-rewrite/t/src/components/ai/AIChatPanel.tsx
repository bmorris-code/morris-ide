...
import { 
  ...
  generateAIResponseStream, // ✅ Use this, not generateAIResponse
...
function CodeBlock({ code, language = 'typescript' }: { code: string; language?: string }) {
  ...
  return (
    ...
    <div className="my-2 rounded-lg overflow-hidden border border-gray-700 bg-[#1e1e1e]">
  ...
  return (
    ...
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''} animate-in fade-in slide-in-from-bottom-2 duration-200`}>
      ...
      <div className={`flex-1 ${isUser ? 'text-right' : ''}`}>
        ...
        <div className={`inline-block max-w-full text-left rounded-xl px-4 py-2.5 text-sm ${
...
export default function AIChatPanel() {
  ...
  if (key && !isProviderInitialized(selectedProvider)) {
  ...
  const handleSaveApiKey = () => {
    ...
    if (apiKeyInput.trim()) {
      ...
      initializeProvider(selectedProvider, apiKeyInput.trim()); // ✅ One call handles all
  ...
  const handleSend = useCallback(async () => {
    ...
    addMessage('user', userContent, codeContext);
    ...
    try {
      ...
      // ✅ NO await here - it's a generator, not a Promise

      const stream = generateAIResponseStream(userContent, codeContext, {
  ...
  }, [input, isGenerating, settings, selectedProvider, apiKeys, addMessage, updateMessage, setGenerating, getActiveTab, selectedText]);
  ...
  const getQuickActions = () => {
    ...
    if (hasSelection) {
      ...
      actions.push(
        ...
        { icon: Code, label: 'Explain Selection', prompt: 'Explain this selected code in detail:\n\n```\n{selection}\n```' },

        { icon: Sparkles, label: 'Refactor', prompt: 'Refactor this code to be cleaner and more efficient:\n\n```\n{selection}\n```' },

        { icon: Zap, label: 'Find Bugs', prompt: 'Find potential bugs or issues in this code:\n\n```\n{selection}\n```' },
          ...
          <div className="mb-3 p-3 bg-yellow-900/15 border border-yellow-800/30 rounded-xl flex items-start gap-2.5">
            ...
            <div className="text-xs text-yellow-400/90">
              ...
              <p className="text-yellow-500/70">
                ...
                Set your {selectedProvider === 'moonshot' ? 'Moonshot' : 'Groq'} API key in settings to use AI features.
