...
export default function LandingPage() {
  const desktopBuildCommands = [
    { label: 'Windows', command: 'npm run electron:build:win' },
    { label: 'macOS', command: 'npm run electron:build:mac' },
    { label: 'Linux', command: 'npm run electron:build:linux' },
  ];
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-[1fr_1fr] gap-8 items-start">
          ...
          <div id="desktop-build" className="bg-[#12121a] border border-gray-800 rounded-xl p-5">
            ...
            <p className="text-xs text-gray-500 mb-4">
              Installer artifacts are generated locally into the release output by Electron Builder. Publish those files to your release host, then wire these cards to the final URLs.
            </p>
            <div className="space-y-3">
              {desktopBuildCommands.map((item) => (
                <div key={item.label} className="p-3 bg-gray-900 rounded-lg border border-gray-800">
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-sm text-gray-200">{item.label}</span>
                    <code className="text-[11px] text-violet-300 bg-gray-950 px-2 py-1 rounded">{item.command}</code>
                  </div>
                </div>
              ))}
            </div>
