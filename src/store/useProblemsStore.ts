...
const parseTerminalProblem = (line: string): Omit<Problem, 'id' | 'timestamp'> | null => {
  ...
  // Python Traceback: File "path/to/file.py", line 42, in <module>
  const pyMatch = trimmed.match(/^File\s+"([^"]+)",\s+line\s+(\d+)(?:,\s+in\s+.+)?$/i);
  if (pyMatch) {
    return {
      source: 'terminal',
      severity: 'error',
  ...
  // Go: path/to/file.go:42:10: message
  const goMatch = trimmed.match(/^(.+\.go):(\d+):(\d+):\s+(.+)$/i);
  if (goMatch) {
    return {
      source: 'terminal',
      severity: 'error',
      filePath: goMatch[1],
      line: Number(goMatch[2]),
      column: Number(goMatch[3]),
      message: goMatch[4],
      raw: line,
    };
  }

  // Rust: error[E0123]: message --> path/to/file.rs:42:10
  const rustMatch = trimmed.match(/-->\s+(.+\.rs):(\d+):(\d+)$/i);
  if (rustMatch) {
    return {
      source: 'terminal',
      severity: 'error',
      filePath: rustMatch[1],
      line: Number(rustMatch[2]),
      column: Number(rustMatch[3]),
      message: 'Rust Compiler Error',
      raw: line,
    };
  }

  if (/\b(error|failed|failure|exception|panic)\b/i.test(trimmed)) {
    return {
